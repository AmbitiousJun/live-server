...
func (remoteM3UHandler) Handle(params HandleParams) (string, error) {
	// 获取环境变量
	url, ok := env.Get(params.UrlEnv)
	if !ok {
		return "", fmt.Errorf("获取不到环境变量: %s", params.UrlEnv)
	}

	// 请求远程 m3u 文本
	resp, err := https.Request(http.MethodGet, url, nil, nil)
	if err != nil {
		return "", fmt.Errorf("请求远程地址失败: %v", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("读取远程响应失败: %v", err)
	}
	infos, err := m3u8.ReadContent(string(bodyBytes))
	if err != nil {
		return "", fmt.Errorf("解析远程响应失败: %v, 原始响应: %s", err, string(bodyBytes))
	}

	// 获取用户请求的频道
	if destInfo, ok := infos[params.ChName]; ok {
		return destInfo.Url, nil
	}

	return "", fmt.Errorf("匹配频道名称失败: %s, 请检查远程地址是否有效", params.ChName)
}
